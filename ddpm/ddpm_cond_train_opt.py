...
'''
ckpt = torch.load(os.path.join("models", args.run_name, f"ckpt_480.pt"))
model.load_state_dict(ckpt)

ema_model.load_state_dict(ckpt)
ema_model = ema_model.eval().requires_grad_(False)
'''
...
def train(args):
    ...
    # model.load_state_dict(torch.load(f"models/{args.run_name}/ckpt_latest.pt"))
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)
    # optimizer.load_state_dict(torch.load(f"models/{args.run_name}/optim_1200.pt"))
    # lr_schedule
...
# CUDA_VISIBLE_DEVICES=3 python ddpm_conditional.py
