...
from models.attunet_r2unet import R2AttU_Net
import time
import matplotlib
matplotlib.use('Agg')
...
from utils import *
from configs.config_attr2u_setting import setting_config
...
def main(config):
    ...
    if config.network == 'attr2u':
        model = R2AttU_Net(img_ch=model_cfg['input_channels'],output_ch=model_cfg['num_classes'],t=3)
    ...
    plt.title('AttR2UNet')
    plt.legend()
    plt.savefig('./log/nn_isic_attr2u_e{}_t{}.png'.format(config.epochs, tkey))
    plt.close()
