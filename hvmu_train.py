...
from utils import *
from configs.config_hvmu_setting import setting_config
...
def main(config):
    ...
    if config.network == 'hvmunet':
        model = HVMUNet(num_classes=model_cfg['num_classes'], 
    ...
    cal_params_flops(model, 256, logger)  # Pass the input size to the function
    ...
    plt.title('HVMUNet')
    plt.legend()
    plt.savefig('./log/nn_isic_hvmunet_e{}_t{}.png'.format(config.epochs, tkey))
    plt.close()
