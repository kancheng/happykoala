...
from models.attunet_r2unet import R2U_Net
import time
import matplotlib
matplotlib.use('Agg')
...
from utils import *
from configs.config_r2u_setting import setting_config
...
def main(config):
    ...
    if config.network == 'r2u':
        model = R2U_Net(img_ch=model_cfg['input_channels'],output_ch=model_cfg['num_classes'],t=3)			
    ...
    plt.title('R2UNet')
    plt.legend()
    plt.savefig('./log/nn_isic_r2u_e{}_t{}.png'.format(config.epochs, tkey))
    plt.close()
