...
from models.attunet_r2unet import AttU_Net
import time
import matplotlib
matplotlib.use('Agg')
...
from utils import *
from configs.config_attu_setting import setting_config
...
def main(config):
    ...
    if config.network == 'attu':
        model = AttU_Net(img_ch=model_cfg['input_channels'],output_ch=model_cfg['num_classes'])
    ...
    plt.title('AttUNet')
    plt.legend()
    plt.savefig('./log/nn_isic_attu_e{}_t{}.png'.format(config.epochs, tkey))
    plt.close()
