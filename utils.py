...
def get_optimizer(config, model):
    assert config.opt in ['Adadelta', 'Adagrad', 'Adam', 'AdamW', 'Adamax', 'ASGD', 'RMSprop', 'Rprop', 'SGD','WPOAdam','WPOSGD'], 'Unsupported optimizer!'
...
def save_imgs(img, msk, msk_pred, i, save_path, datasets, threshold=0.5, test_data_name=None):
    if test_data_name is not None:
        # 假设 test_data_name 是 Namespace 对象，提取字符串属性
        test_data_name_str = test_data_name.name  # 根据实际属性名进行修改
        save_path = save_path + test_data_name_str + '_'
...
class BCELoss2(nn.Module):
    def __init__(self):
        ...
        # 改成 BCEWithLogitsLoss，自動處理 sigmoid
