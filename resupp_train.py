...
from utils import *
from configs.config_resupp_setting import setting_config
...
def main(config):
    ...
    if config.network == 'resunetpp':
        model = ResUnetPlusPlus(
            channel=model_cfg['input_channels']
        )
    ...
    plt.title('RESUNetpp')
    plt.legend()
    plt.savefig('./log/nn_isic_resunetpp_e{}_t{}.png'.format(config.epochs, tkey))
    plt.close()
